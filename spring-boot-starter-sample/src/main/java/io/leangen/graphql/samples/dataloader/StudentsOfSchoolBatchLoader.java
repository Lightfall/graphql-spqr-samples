...
@Component
public class StudentsOfSchoolBatchLoader implements BatchLoader<Long, List<StudentDto>> {

   private final StudentRepo studentRepo;

    public StudentsOfSchoolBatchLoader(StudentRepo studentRepo) {
    ...
    public CompletionStage<List<List<StudentDto>>> load(List<Long> list) {
        return CompletableFuture.supplyAsync(() -> {
            Map<Long,List<Student>> data = studentRepo.getStudentsOfSchools(list);
            return list.stream()
